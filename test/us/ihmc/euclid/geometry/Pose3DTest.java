...
public class Pose3DTest
{
   ...
   public void testSetToNaN()
   {
      ...
      for (int i = 0; i < ITERATIONS; ++i)
      {
         toSet = EuclidGeometryRandomTools.generateRandomPose3D(random);
   ...
   public void testSetComponents()
   {
      ...
      for (int i = 0; i < ITERATIONS; ++i)
      {
         ...
         aa = EuclidCoreRandomTools.generateRandomAxisAngle(random);
         ...
         toSet = EuclidGeometryRandomTools.generateRandomPose3D(random);
         rot = EuclidCoreRandomTools.generateRandomRotationMatrix(random);
   ...
   public void testEquals()
   {
      ...
      { // Poses are equal if and only if point components are exactly equal
         ...
         translation = EuclidCoreRandomTools.generateRandomVector3DWithFixedLength(random, epsilon);
         secondPose.appendTranslation(translation);
      ...
      { // Poses are equal if and only if orientation angles are exactly equal
         ...
         orthogonal = EuclidCoreRandomTools.generateRandomOrthogonalVector3D(random, normal, true);
