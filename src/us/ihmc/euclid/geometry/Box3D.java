...
public class Box3D extends Shape3D<Box3D>
{
   ...
   protected double evaluateQuery(double x, double y, double z, Point3DBasics closestPointToPack, Vector3DBasics normalToPack)
   {
      ...
      else
      {
         ...
         double xClamped = EuclidCoreTools.clamp(x, halfSize.getX());
         double yClamped = EuclidCoreTools.clamp(y, halfSize.getY());
         double zClamped = EuclidCoreTools.clamp(z, halfSize.getZ());
